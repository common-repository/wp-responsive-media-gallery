<?php
/*
 * vsz_thumb - WP Image Resizer v1.3
 *
...
if ( !function_exists( 'vsz_thumb' ) ) {
	function vsz_thumb($url, $params = array(), $single = true) {
		$class = VSZ_Class_Factory::getNewestVersion( 'VSZ_Thumb' );
		return call_user_func( array( $class, 'thumb' ), $url, $params, $single );
	}
}

/* if ( !function_exists( 'vsz_thumb_custom' ) ) {
	function vsz_thumb_custom($url, $params = array(), $single = true) {
		$class = VSZ_Class_Factory::getNewestVersion( 'VSZ_Thumb' );
		return call_user_func( array( $class, 'thumb' ), $url, $params, $single );
	}
} */
...
/**
 * Class factory, this is to make sure that when multiple vsz_thumb scripts
 * are used (e.g. a plugin and a theme both use it), we always use the
 * latest version.
 */
if ( !class_exists( 'VSZ_Class_Factory' ) ) {
class VSZ_Class_Factory {
 ...
 */
add_filter('wp_image_editors', 'vsz_wp_image_editor');

// Instead of using the default image editors, use our extended ones
if ( !function_exists( 'vsz_wp_image_editor' ) ) {
function vsz_wp_image_editor($editorArray) {
    // Make sure that we use the latest versions
    return array(
        VSZ_Class_Factory::getNewestVersion( 'VSZ_Image_Editor_GD' ),
        VSZ_Class_Factory::getNewestVersion( 'VSZ_Image_Editor_Imagick' ),
    );
}
}
 ...
 */
add_action('admin_init', 'vsz_wp_image_editor_check');
if ( !function_exists( 'vsz_wp_image_editor_check' ) ) {
function vsz_wp_image_editor_check() {
    $arg = array('mime_type' => 'image/jpeg');
    if (wp_image_editor_supports($arg) !== true) {
        add_filter('admin_notices', 'vsz_wp_image_editor_check_notice');
    }
}
}
if ( !function_exists( 'vsz_wp_image_editor_check_notice' ) ) {
function vsz_wp_image_editor_check_notice() {
 ...
 */
if ( !class_exists( 'VSZ_Image_Editor_Imagick_1_3' ) ) {
VSZ_Class_Factory::addClassVersion( 'VSZ_Image_Editor_Imagick', 'VSZ_Image_Editor_Imagick_1_3', '1.3' );

class VSZ_Image_Editor_Imagick_1_3 extends WP_Image_Editor_Imagick {
 ...
 */
if ( !class_exists( 'VSZ_Image_Editor_GD_1_3' ) ) {
VSZ_Class_Factory::addClassVersion( 'VSZ_Image_Editor_GD', 'VSZ_Image_Editor_GD_1_3', '1.3' );

class VSZ_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
 ...
 * Main Class
 */
if ( !class_exists( 'VSZ_Thumb_1_3' ) ) {
VSZ_Class_Factory::addClassVersion( 'VSZ_Thumb', 'VSZ_Thumb_1_3', '1.3' );
...
class VSZ_Thumb_1_3 {
    ...
    public static function thumb($url, $params = array(), $single = true) {
		...
		$plugin_url = dirname(plugin_dir_url(__FILE__))."/admin/images";
		$plugin_dir = dirname(plugin_dir_path(__FILE__))."/admin/images";
		...
		// #3 check if the image is in the gallery plugin image folder
		} else if (strpos($url, $plugin_url) !== false) {
            $rel_path = str_replace( $plugin_url, '', $url);
            $img_path = $plugin_dir . $rel_path;
		}
        ...
        if ( defined( 'VSZTHUMB_UPLOAD_DIR' ) ) {
            $upload_dir .= "/" . VSZTHUMB_UPLOAD_DIR;
            $upload_url .= "/" . VSZTHUMB_UPLOAD_DIR;
        } else {
            $upload_dir .= "/vsz_thumb";
            $upload_url .= "/vsz_thumb";
        }
...
add_filter('image_resize_dimensions', 'vsz_image_resize_dimensions', 10, 5);
if ( !function_exists( 'vsz_image_resize_dimensions' ) ) {
function vsz_image_resize_dimensions($payload, $orig_w, $orig_h, $dest_w, $dest_h, $crop = false) {
...
// so that you won't have to use the function vsz_thumb in order to do resizing.
// To make this work, in the WP image functions, when specifying an
// array for the image dimensions, add a 'vsz_thumb' => true to
// the array, then add your normal $params arguments.
//
// e.g. the_post_thumbnail( array( 1024, 400, 'vsz_thumb' => true, 'grayscale' => true ) );
add_filter( 'image_downsize', 'vsz_image_downsize', 1, 3 );
if ( !function_exists( 'vsz_image_downsize' ) ) {
function vsz_image_downsize( $out, $id, $size ) {
    if ( ! is_array( $size ) ) {
        return false;
    }
    if ( ! array_key_exists( 'vsz_thumb', $size ) ) {
        return false;
    }
    if ( empty( $size['vsz_thumb'] ) ) {
        return false;
    }
    ...
    $resized_img_url = vsz_thumb( $img_url, $params );
