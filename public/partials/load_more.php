...
foreach($arrgalleryRecords->posts as $key =>$value){
		...
		if(isset($arr_details) && !empty($arr_details)){
			...
			/////// Inner forloop starts here  ///////////
				...
				///////////////// For Image Section  /////////////
				if($elementInfo['content_type'] == "image" && $elementInfo['img_id'] != '' && $elementInfo['exclude'] != 1){
				...
				///////////////// For Video  Section  /////////////
				if($elementInfo['content_type'] == "video" && !empty($elementInfo['video_url']) && $elementInfo['exclude'] != 1){
					$content .= '<div class="col-grid-'.$noOfCols.' sel-equal-box">';
						$content .= '<div class="image-outer">
										<!-- style is for hover color -->
										<style>
					...
					$sampleUrl = $elementInfo['thumb_url'];										/// Getting thumb image url
