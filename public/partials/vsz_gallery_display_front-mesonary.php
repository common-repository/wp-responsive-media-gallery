	...
	$content .='<div class="rmg_wrap">
					<div class="gallery-plugin">
						<div class="gallery-outer magnific-all">
							<div class="masonry_grid js-masonry">';
	...
	foreach($arrgalleryRecords->posts as $key =>$value){
		...
		if(isset($arr_details) && !empty($arr_details)){
			...
			foreach($arr_details as $popupkey => $elementInfo){
				...
				$popupkey = $value->ID."-".$popupkey;	
				$temp = 0;
				/////// Creating a unique popup key to differentiate every popup div
				///////////////// For Image Section  /////////////
	...
	$content .= '</div>
		...
		jQuery(document).ready(function($){
			...
			jQuery(".vsz_btn-popup-gallery").magnificPopup({
				...
				callbacks: {
					buildControls: function() {
					  ...
					  //this.contentContainer.find(".popup-gallery").append( "<p>Test</p>" );
					},
					change: function() {
						///// Paise video code
			...
			jQuery("#loading").click(function(){
					baseUrl = "'.plugin_dir_url(__FILE__).'"+"load_more_masonry.php";
					...
					jQuery.ajax({
						...
						data: {"layout":"masonry","load_more":loadmore,"postId":postId,"galleryCategoryName":galleryCategoryName,"action":"load_more_images_masonary","ajax_nonce":ajax_nonce,"loadmoreitem":loadmoreitem},
						success: function(data){
							
							loadmore= loadmore + loadmoreitem;
							jQuery(".masonry_grid").append(data);
							setTimeout(function(){
								//jQuery(".masonry_grid").masonry("reloadItems").masonry("layout");
							},1000);
							function resetProgress() {
  $status.css({ opacity: 1 });
  loadedImageCount = 0;
